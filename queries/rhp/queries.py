...
query_documento_clinico = "SELECT CD_OBJETO, cd_atendimento, cd_tipo_documento, tp_status, DH_CRIACAO FROM DBAMV.VW_EXIMIO_PW_DOCUMENTO_CLINICO WHERE DH_CRIACAO BETWEEN TO_DATE('{data_ini}', 'DD/MM/YYYY') AND TO_DATE('{data_fim}', 'DD/MM/YYYY') AND CD_TIPO_DOCUMENTO = 36"
...
query_tempo_processo = "SELECT DH_PROCESSO, CD_TIPO_TEMPO_PROCESSO, CD_ATENDIMENTO FROM DBAMV.VW_EXIMIO_SACR_TEMPO_PROCESSO WHERE DH_PROCESSO BETWEEN TO_DATE('{data_ini}', 'DD/MM/YYYY') AND TO_DATE('{data_fim}', 'DD/MM/YYYY') AND CD_TIPO_TEMPO_PROCESSO IN (1, 10, 11, 12, 21, 22, 31, 90)"
...
query_triagem_atendimento = "SELECT CD_ATENDIMENTO, CD_TRIAGEM_ATENDIMENTO, CD_SINTOMA_AVALIACAO, DS_SENHA, DH_PRE_ATENDIMENTO FROM DBAMV.VW_EXIMIO_TRIAGEM_ATENDIMENTO WHERE DH_PRE_ATENDIMENTO BETWEEN TO_DATE('{data_ini}', 'DD/MM/YYYY') AND TO_DATE('{data_fim}', 'DD/MM/YYYY')"

query_usuario = 'SELECT CD_USUARIO, NM_USUARIO FROM DBAMV.VW_EXIMIO_USUARIOS'

query_pre_med = "SELECT CD_PRE_MED, PM.CD_ATENDIMENTO, PM.CD_PRESTADOR, CD_DOCUMENTO_CLINICO, DT_PRE_MED, TP_PRE_MED, PM.CD_SETOR FROM DBAMV.VW_EXIMIO_PRE_MED PM WHERE DT_PRE_MED BETWEEN TO_DATE('{data_ini}', 'DD/MM/YYYY') AND TO_DATE('{data_fim}', 'DD/MM/YYYY')"
...
query_reg_fat = "SELECT CD_REG_FAT, SN_FECHADA, DT_INICIO, DT_FINAL, DT_FECHAMENTO, CD_REMESSA, VL_TOTAL_CONTA, RF.CD_ATENDIMENTO FROM DBAMV.VW_EXIMIO_REG_FAT RF WHERE DT_INICIO BETWEEN TO_DATE('{data_ini}', 'DD/MM/YYYY') AND TO_DATE('{data_fim}', 'DD/MM/YYYY')"
...
query_custo_final = "SELECT VL_CUSTO_CENCIR, DT_COMPETENCIA FROM DBAMV.VW_EXIMIO_CUSTO_FINAL WHERE DT_COMPETENCIA BETWEEN TO_DATE('{data_ini}', 'DD/MM/YYYY') AND TO_DATE('{data_fim}', 'DD/MM/YYYY')"

query_mvto_estoque = "SELECT CD_MVTO_ESTOQUE, ME.CD_SETOR, ME.CD_ATENDIMENTO, CD_MOT_DEV, ME.CD_MULTI_EMPRESA, DT_MVTO_ESTOQUE FROM DBAMV.VW_EXIMIO_MVTO_ESTOQUE ME WHERE DT_MVTO_ESTOQUE BETWEEN TO_DATE('{data_ini}', 'DD/MM/YYYY') AND TO_DATE('{data_fim}', 'DD/MM/YYYY')"
...
query_remessa_fatura = "SELECT RF.CD_REMESSA, DT_ABERTURA, RF.DT_FECHAMENTO, DT_ENTREGA_DA_FATURA FROM DBAMV.VW_EXIMIO_REMESSA_FATURA RF WHERE DT_ABERTURA BETWEEN TO_DATE('{data_ini}', 'DD/MM/YYYY') AND TO_DATE('{data_fim}', 'DD/MM/YYYY')"

query_repasse = "SELECT DISTINCT R.CD_REPASSE, DT_COMPETENCIA FROM DBAMV.VW_EXIMIO_REPASSE R WHERE DT_COMPETENCIA BETWEEN TO_DATE('{data_ini}', 'DD/MM/YYYY') AND TO_DATE('{data_fim}', 'DD/MM/YYYY')"

query_it_repasse = 'SELECT DISTINCT ITR.CD_REG_FAT, CD_LANCAMENTO_FAT, CD_REPASSE FROM DBAMV.VW_EXIMIO_IT_REPASSE ITR'

query_itent_pro = "SELECT VL_TOTAL, ITP.CD_ATENDIMENTO, CD_PRODUTO, VL_UNITARIO, DT_GRAVACAO FROM DBAMV.VW_EXIMIO_ITENT_PRO ITP WHERE DT_GRAVACAO BETWEEN TO_DATE('{data_ini}', 'DD/MM/YYYY') AND TO_DATE('{data_fim}', 'DD/MM/YYYY')"
...
query_custo_medio_mensal = "SELECT VL_CUSTO_MEDIO, DH_CUSTO_MEDIO, CD_PRODUTO, CD_MULTI_EMPRESA FROM DBAMV.VW_EXIMIO_CUSTO_MEDIO_MENSAL WHERE DH_CUSTO_MEDIO BETWEEN TO_DATE('{data_ini}', 'DD/MM/YYYY') AND TO_DATE('{data_fim}', 'DD/MM/YYYY')"
