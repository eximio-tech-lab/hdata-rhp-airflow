query_atendime_hdata = "SELECT A.CD_MULTI_EMPRESA, CD_PACIENTE, CD_ATENDIMENTO, CD_CID, CD_MOT_ALT, CD_TIP_RES, CD_CONVENIO, CD_ESPECIALID, CD_PRESTADOR, CD_ATENDIMENTO_PAI, CD_LEITO, A.CD_ORI_ATE, CD_SERVICO, TP_ATENDIMENTO, DT_ATENDIMENTO, HR_ATENDIMENTO, HR_ALTA, HR_ALTA_MEDICA, CD_TIP_MAR, CD_SINTOMA_AVALIACAO, NM_USUARIO_ALTA_MEDICA FROM MV_RHP.ATENDIME A WHERE DT_ATENDIMENTO = TO_DATE('{data_ini}', 'DD/MM/YYYY')"

query_cid_hdata = 'SELECT CD_CID, DS_CID, CD_SGRU_CID FROM MV_RHP.CID'

query_classificacao_risco_hdata = "SELECT CD_CLASSIFICACAO_RISCO, CD_COR_REFERENCIA, CD_TRIAGEM_ATENDIMENTO, DH_CLASSIFICACAO_RISCO FROM MV_RHP.SACR_CLASSIFICACAO_RISCO WHERE DH_CLASSIFICACAO_RISCO BETWEEN TO_DATE('{data_ini}', 'DD/MM/YYYY') AND TO_DATE('{data_ini}', 'DD/MM/YYYY') + INTERVAL '1' DAY"
...
query_documento_clinico_hdata = "SELECT CD_OBJETO, cd_atendimento, cd_tipo_documento, tp_status, DH_CRIACAO FROM MV_RHP.PW_DOCUMENTO_CLINICO WHERE DH_CRIACAO BETWEEN TO_DATE('{data_ini}', 'DD/MM/YYYY') AND TO_DATE('{data_ini}', 'DD/MM/YYYY') + INTERVAL '1' DAY AND CD_TIPO_DOCUMENTO = 36"
...
query_registro_alta_hdata = "SELECT HR_ALTA_MEDICA, CD_ATENDIMENTO FROM MV_RHP.PW_REGISTRO_ALTA WHERE HR_ALTA_MEDICA BETWEEN TO_DATE('{data_ini}', 'DD/MM/YYYY') AND TO_DATE('{data_ini}', 'DD/MM/YYYY') + INTERVAL '1' DAY"
...
query_tempo_processo_hdata = "SELECT DH_PROCESSO, CD_TIPO_TEMPO_PROCESSO, CD_ATENDIMENTO FROM MV_RHP.SACR_TEMPO_PROCESSO WHERE DH_PROCESSO BETWEEN TO_DATE('{data_ini}', 'DD/MM/YYYY') AND TO_DATE('{data_ini}', 'DD/MM/YYYY') + INTERVAL '1' DAY AND CD_TIPO_TEMPO_PROCESSO IN (1, 10, 11, 12, 21, 22, 31, 90)"
...
query_triagem_atendimento_hdata = "SELECT CD_ATENDIMENTO, CD_TRIAGEM_ATENDIMENTO, CD_SINTOMA_AVALIACAO, DS_SENHA, DH_PRE_ATENDIMENTO FROM MV_RHP.TRIAGEM_ATENDIMENTO WHERE DH_PRE_ATENDIMENTO BETWEEN TO_DATE('{data_ini}', 'DD/MM/YYYY') AND TO_DATE('{data_ini}', 'DD/MM/YYYY') + INTERVAL '1' DAY"

query_usuario_hdata = 'SELECT CD_USUARIO, NM_USUARIO FROM MV_RHP.USUARIOS'

query_pre_med_hdata = "SELECT CD_PRE_MED, PM.CD_ATENDIMENTO, PM.CD_PRESTADOR, CD_DOCUMENTO_CLINICO, DT_PRE_MED, TP_PRE_MED, PM.CD_SETOR FROM MV_RHP.PRE_MED PM WHERE DT_PRE_MED BETWEEN TO_DATE('{data_ini}', 'DD/MM/YYYY') AND TO_DATE('{data_ini}', 'DD/MM/YYYY') + INTERVAL '1' DAY"
...
query_itreg_amb_hdata = "SELECT IRA.CD_ATENDIMENTO, CD_PRO_FAT, CD_REG_AMB, CD_GRU_FAT, CD_LANCAMENTO, QT_LANCAMENTO, VL_UNITARIO, VL_NOTA, IRA.CD_SETOR, CD_SETOR_PRODUZIU, TP_PAGAMENTO, SN_PERTENCE_PACOTE, VL_TOTAL_CONTA, SN_FECHADA, DT_FECHAMENTO, CD_ITMVTO FROM MV_RHP.ITREG_AMB IRA WHERE DT_FECHAMENTO BETWEEN TO_DATE('{data_ini}', 'DD/MM/YYYY') AND TO_DATE('{data_ini}', 'DD/MM/YYYY') + INTERVAL '1' DAY"

query_reg_fat_hdata = "SELECT CD_REG_FAT, SN_FECHADA, DT_INICIO, DT_FINAL, DT_FECHAMENTO, CD_REMESSA, VL_TOTAL_CONTA, RF.CD_ATENDIMENTO FROM MV_RHP.REG_FAT RF WHERE DT_INICIO BETWEEN TO_DATE('{data_ini}', 'DD/MM/YYYY') AND TO_DATE('{data_ini}', 'DD/MM/YYYY') + INTERVAL '1' DAY"

query_itreg_fat_hdata = "SELECT IRF.CD_REG_FAT, CD_LANCAMENTO, DT_LANCAMENTO, QT_LANCAMENTO, TP_PAGAMENTO, VL_UNITARIO, VL_NOTA, IRF.CD_CONTA_PAI, CD_PRO_FAT, CD_GRU_FAT, IRF.VL_TOTAL_CONTA, SN_PERTENCE_PACOTE, IRF.CD_SETOR, CD_SETOR_PRODUZIU, CD_ITMVTO FROM MV_RHP.ITREG_FAT IRF WHERE DT_LANCAMENTO BETWEEN TO_DATE('{data_ini}', 'DD/MM/YYYY') AND TO_DATE('{data_ini}', 'DD/MM/YYYY') + INTERVAL '1' DAY"

query_custo_final_hdata = "SELECT VL_CUSTO_CENCIR, DT_COMPETENCIA FROM MV_RHP.CUSTO_FINAL WHERE DT_COMPETENCIA BETWEEN TO_DATE('{data_ini}', 'DD/MM/YYYY') AND TO_DATE('{data_ini}', 'DD/MM/YYYY') + INTERVAL '1' DAY"

query_mvto_estoque_hdata = "SELECT CD_MVTO_ESTOQUE, ME.CD_SETOR, ME.CD_ATENDIMENTO, CD_MOT_DEV, ME.CD_MULTI_EMPRESA, DT_MVTO_ESTOQUE FROM MV_RHP.MVTO_ESTOQUE ME WHERE DT_MVTO_ESTOQUE BETWEEN TO_DATE('{data_ini}', 'DD/MM/YYYY') AND TO_DATE('{data_ini}', 'DD/MM/YYYY') + INTERVAL '1' DAY"
...
query_remessa_fatura_hdata = "SELECT RF.CD_REMESSA, DT_ABERTURA, RF.DT_FECHAMENTO, DT_ENTREGA_DA_FATURA FROM MV_RHP.REMESSA_FATURA RF WHERE DT_ABERTURA BETWEEN TO_DATE('{data_ini}', 'DD/MM/YYYY') AND TO_DATE('{data_ini}', 'DD/MM/YYYY') + INTERVAL '1' DAY"

query_repasse_hdata = "SELECT DISTINCT R.CD_REPASSE, DT_COMPETENCIA FROM MV_RHP.REPASSE R WHERE DT_COMPETENCIA BETWEEN TO_DATE('{data_ini}', 'DD/MM/YYYY') AND TO_DATE('{data_ini}', 'DD/MM/YYYY') + INTERVAL '1' DAY"

query_it_repasse_hdata = 'SELECT DISTINCT ITR.CD_REG_FAT, CD_LANCAMENTO_FAT, CD_REPASSE FROM MV_RHP.IT_REPASSE ITR'

query_itent_pro_hdata = "SELECT VL_TOTAL, ITP.CD_ATENDIMENTO, CD_PRODUTO, VL_UNITARIO, DT_GRAVACAO FROM MV_RHP.ITENT_PRO ITP WHERE DT_GRAVACAO BETWEEN TO_DATE('{data_ini}', 'DD/MM/YYYY') AND TO_DATE('{data_ini}', 'DD/MM/YYYY') + INTERVAL '1' DAY"
...
query_custo_medio_mensal_hdata = "SELECT VL_CUSTO_MEDIO, DH_CUSTO_MEDIO, CD_PRODUTO, CD_MULTI_EMPRESA FROM MV_RHP.CUSTO_MEDIO_MENSAL WHERE DH_CUSTO_MEDIO BETWEEN TO_DATE('{data_ini}', 'DD/MM/YYYY') AND TO_DATE('{data_ini}', 'DD/MM/YYYY') + INTERVAL '1' DAY"
