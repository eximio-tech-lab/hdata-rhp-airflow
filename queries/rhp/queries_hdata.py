...
query_classificacao_risco_hdata = "SELECT CD_CLASSIFICACAO_RISCO, CD_COR_REFERENCIA, CD_TRIAGEM_ATENDIMENTO, DH_CLASSIFICACAO_RISCO FROM MV_RHP.SACR_CLASSIFICACAO_RISCO WHERE DH_CLASSIFICACAO_RISCO = TO_DATE('{data_ini}', 'DD/MM/YYYY')"
...
query_documento_clinico_hdata = "SELECT CD_OBJETO, cd_atendimento, cd_tipo_documento, tp_status, DH_CRIACAO FROM MV_RHP.PW_DOCUMENTO_CLINICO WHERE DH_CRIACAO = TO_DATE('{data_ini}', 'DD/MM/YYYY') AND CD_TIPO_DOCUMENTO = 36"
...
query_registro_alta_hdata = "SELECT HR_ALTA_MEDICA, CD_ATENDIMENTO FROM MV_RHP.PW_REGISTRO_ALTA WHERE HR_ALTA_MEDICA = TO_DATE('{data_ini}', 'DD/MM/YYYY')"
...
query_tempo_processo_hdata = "SELECT DH_PROCESSO, CD_TIPO_TEMPO_PROCESSO, CD_ATENDIMENTO FROM MV_RHP.SACR_TEMPO_PROCESSO WHERE DH_PROCESSO = TO_DATE('{data_ini}', 'DD/MM/YYYY') AND CD_TIPO_TEMPO_PROCESSO IN (1, 10, 11, 12, 21, 22, 31, 90)"
...
query_triagem_atendimento_hdata = "SELECT CD_ATENDIMENTO, CD_TRIAGEM_ATENDIMENTO, CD_SINTOMA_AVALIACAO, DS_SENHA, DH_PRE_ATENDIMENTO FROM MV_RHP.TRIAGEM_ATENDIMENTO WHERE DH_PRE_ATENDIMENTO = TO_DATE('{data_ini}', 'DD/MM/YYYY')"

query_usuario_hdata = 'SELECT CD_USUARIO, NM_USUARIO FROM MV_RHP.USUARIOS'

query_pre_med_hdata = "SELECT CD_PRE_MED, PM.CD_ATENDIMENTO, PM.CD_PRESTADOR, CD_DOCUMENTO_CLINICO, DT_PRE_MED, TP_PRE_MED, PM.CD_SETOR FROM MV_RHP.PRE_MED PM WHERE DT_PRE_MED = TO_DATE('{data_ini}', 'DD/MM/YYYY')"
...
query_reg_fat_hdata = "SELECT CD_REG_FAT, SN_FECHADA, DT_INICIO, DT_FINAL, DT_FECHAMENTO, CD_REMESSA, VL_TOTAL_CONTA, RF.CD_ATENDIMENTO FROM MV_RHP.REG_FAT RF WHERE DT_INICIO = TO_DATE('{data_ini}', 'DD/MM/YYYY')"
...
query_custo_final_hdata = "SELECT VL_CUSTO_CENCIR, DT_COMPETENCIA FROM MV_RHP.CUSTO_FINAL WHERE DT_COMPETENCIA = TO_DATE('{data_ini}', 'DD/MM/YYYY')"

query_mvto_estoque_hdata = "SELECT CD_MVTO_ESTOQUE, ME.CD_SETOR, ME.CD_ATENDIMENTO, CD_MOT_DEV, ME.CD_MULTI_EMPRESA, DT_MVTO_ESTOQUE FROM MV_RHP.MVTO_ESTOQUE ME WHERE DT_MVTO_ESTOQUE = TO_DATE('{data_ini}', 'DD/MM/YYYY')"
...
query_remessa_fatura_hdata = "SELECT RF.CD_REMESSA, DT_ABERTURA, RF.DT_FECHAMENTO, DT_ENTREGA_DA_FATURA FROM MV_RHP.REMESSA_FATURA RF WHERE DT_ABERTURA = TO_DATE('{data_ini}', 'DD/MM/YYYY')"

query_repasse_hdata = "SELECT DISTINCT R.CD_REPASSE, DT_COMPETENCIA FROM MV_RHP.REPASSE R WHERE DT_COMPETENCIA = TO_DATE('{data_ini}', 'DD/MM/YYYY')"

query_it_repasse_hdata = 'SELECT DISTINCT ITR.CD_REG_FAT, CD_LANCAMENTO_FAT, CD_REPASSE FROM MV_RHP.IT_REPASSE ITR'

query_itent_pro_hdata = "SELECT VL_TOTAL, ITP.CD_ATENDIMENTO, CD_PRODUTO, VL_UNITARIO, DT_GRAVACAO FROM MV_RHP.ITENT_PRO ITP WHERE DT_GRAVACAO = TO_DATE('{data_ini}', 'DD/MM/YYYY')"
...
query_custo_medio_mensal_hdata = "SELECT VL_CUSTO_MEDIO, DH_CUSTO_MEDIO, CD_PRODUTO, CD_MULTI_EMPRESA FROM MV_RHP.CUSTO_MEDIO_MENSAL WHERE DH_CUSTO_MEDIO = TO_DATE('{data_ini}', 'DD/MM/YYYY')"
