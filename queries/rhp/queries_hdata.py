...
query_documento_clinico_hdata = "SELECT CD_OBJETO, cd_atendimento, cd_tipo_documento, tp_status, DH_CRIACAO FROM MV_RHP.PW_DOCUMENTO_CLINICO WHERE DH_CRIACAO >= TO_DATE('{data_ini}', 'DD/MM/YYYY') AND DH_CRIACAO < TO_DATE('{data_fim}', 'DD/MM/YYYY') + INTERVAL '1' DAY AND CD_TIPO_DOCUMENTO = 36"
...
query_tempo_processo_hdata = "SELECT DH_PROCESSO, CD_TIPO_TEMPO_PROCESSO, CD_ATENDIMENTO FROM MV_RHP.SACR_TEMPO_PROCESSO WHERE DH_PROCESSO >= TO_DATE('{data_ini}', 'DD/MM/YYYY') AND DH_PROCESSO < TO_DATE('{data_fim}', 'DD/MM/YYYY') + INTERVAL '1' DAY AND CD_TIPO_TEMPO_PROCESSO IN (1, 10, 11, 12, 21, 22, 31, 90)"
...
query_pre_med_hdata = "SELECT CD_PRE_MED, PM.CD_ATENDIMENTO, PM.CD_PRESTADOR, CD_DOCUMENTO_CLINICO, DT_PRE_MED, TP_PRE_MED, PM.CD_SETOR FROM MV_RHP.PRE_MED PM WHERE DT_PRE_MED >= TO_DATE('{data_ini}', 'DD/MM/YYYY') AND DT_PRE_MED < TO_DATE('{data_fim}', 'DD/MM/YYYY') + INTERVAL '1' DAY"
...
query_ped_rx = "SELECT CD_PED_RX, DT_PEDIDO, CD_ATENDIMENTO, CD_PRESTADOR, NM_USUARIO, CD_SET_EXA, NM_SET_EXA, CD_SETOR, CD_PRE_MED FROM MV_RHP.PED_RX WHERE DT_PEDIDO >= TO_DATE('{data_ini}', 'DD/MM/YYYY') AND DT_PEDIDO < TO_DATE('{data_fim}', 'DD/MM/YYYY') + INTERVAL '1' DAY"

query_ped_lab = "SELECT CD_PED_LAB, DT_PEDIDO, CD_ATENDIMENTO, CD_PRESTADOR, NM_USUARIO, CD_TECNICO_EXA, TP_SOLICITACAO, CD_SET_EXA, NM_SET_EXA, CD_SETOR, CD_PRE_MED FROM MV_RHP.PED_LAB WHERE DT_PEDIDO >= TO_DATE('{data_ini}', 'DD/MM/YYYY') AND DT_PEDIDO < TO_DATE('{data_fim}', 'DD/MM/YYYY') + INTERVAL '1' DAY"

query_fech_chec = "SELECT CD_FECHAMENTO_HORARIO_CHECAGEM, CD_FECHAMENTO, CD_ITPRE_MED, CD_USUARIO, DH_CHECAGEM, SN_ALTERADO, SN_SUSPENSO FROM MV_RHP.PW_HR_FECHADO_CHEC WHERE DH_CHECAGEM >= TO_DATE('{data_ini}', 'DD/MM/YYYY') AND DH_CHECAGEM < TO_DATE('{data_fim}', 'DD/MM/YYYY') + INTERVAL '1' DAY"

query_itped_lab = "SELECT CD_PED_LAB, CD_ITPED_LAB, CD_EXA_LAB, NM_EXA_LAB, CD_ATENDIMENTO, SN_REALIZADO, CD_LABORATORIO, CD_SET_EXA, NM_SET_EXA, CD_MATERIAL, CD_ITPRE_MED FROM MV_RHP.ITPED_LAB WHERE CD_PED_LAB IN ({cd_ped_lab})"

query_itped_rx = "SELECT CD_PED_RX, CD_ITPED_RX, CD_EXA_RX, CD_LAUDO, SN_REALIZADO, CD_ITPRE_MED, DS_LAUDO FROM MV_RHP.ITPED_RX WHERE CD_PED_RX IN ({cd_ped_rx})"

query_material = "SELECT CD_MATERIAL, DS_MATERIAL FROM MV_RHP.MATERIAL"

query_mod_exame = "SELECT CD_MODALIDADE_EXAME, DS_MODALIDADE_EXAME FROM MV_RHP.MOD_EXAME"

query_leito = "SELECT CD_LEITO, CD_UNID_INT, DS_ENFERMARIA, DS_LEITO, TP_SITUACAO FROM MV_RHP.LEITO"

query_unid_int = "SELECT CD_UNID_INT, DS_UNID_INT, DS_LOCALIZACAO, CD_SETOR, SN_ATIVO FROM MV_RHP.UNID_INT"

query_mov_int = "SELECT CD_MOV_INT, CD_ATENDIMENTO, CD_CONVENIO, CD_PRESTADOR, CD_LEITO, DT_MOV_INT, HR_MOV_INT, DS_MOTIVO, SN_RESERVA, CD_LEITO_ANTERIOR, CD_TIP_ACOM, NM_USUARIO FROM MV_RHP.MOV_INT WHERE DT_MOV_INT >= TO_DATE('{data_ini}', 'DD/MM/YYYY') AND DT_MOV_INT < TO_DATE('{data_fim}', 'DD/MM/YYYY') + INTERVAL '1' DAY"

query_tip_acom = "SELECT CD_TIP_ACOM, DS_TIP_ACOM, VL_FATOR_CUSTO, TP_ACOMODACAO FROM MV_RHP.TIP_ACOM"