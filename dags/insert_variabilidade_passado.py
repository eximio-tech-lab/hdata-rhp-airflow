...
def df_mov_exame():
    ...
    for dt in rrule.rrule(rrule.MONTHLY, dtstart=dt_ini, until=dt_ontem):
        ...
        df_dim = pd.read_sql(query_mov_exame.format(data_ini=dt.strftime('%d/%m/%Y'), data_fim=data_fim.strftime('%d/%m/%Y')), connect_rhp())
        ...
        df_stage = pd.read_sql(query_mov_exame_hdata.format(data_ini=dt.strftime('%d/%m/%Y'), data_fim=data_fim.strftime('%d/%m/%Y')), connect_rhp_hdata())
