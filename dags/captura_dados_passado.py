...
def df_tempo_processo():
    ...
    for dt in rrule.rrule(rrule.DAILY, dtstart=datetime.datetime(2019, 1, 1), until=dt_ontem):
        ...
        df_stage = pd.read_sql(query_temp_tempo_processo_hdata.format(data_ini=data_1.strftime('%d/%m/%Y'), data_fim=data_2.strftime('%d/%m/%Y')), connect_rhp_hdata())
        ...
        sql="INSERT INTO MV_RHP.TEMP_SACR_TEMPO_PROCESSO (DH_PROCESSO, CD_TIPO_TEMPO_PROCESSO, CD_ATENDIMENTO, NM_USUARIO) VALUES (:1, :2, :3, :4)"
...
def df_mov_int():
    ...
    for dt in rrule.rrule(rrule.DAILY, dtstart=datetime.datetime(2020, 11, 1), until=dt_ontem):
        data_1 = dt
        data_2 = dt
